...
class GAN:
    ...
    def __init__(self, fraction_to_use = 1.0):
        ...
        self.gnet_W1 = tf.Variable(tf.truncated_normal(shape = [100, 128], stddev = 0.02))
        self.gnet_b1 = tf.Variable(tf.constant(value = 0.0, shape = [128]))
        self.gnet_W2 = tf.Variable(tf.truncated_normal(shape = [128, 784], stddev = 0.02))
        self.gnet_b2 = tf.Variable(tf.constant(value = 0.0, shape = [784]))
        
        # Generative connections
        self.gnet_z = tf.placeholder(dtype=tf.float32, shape=[None, 100])
        self.gnet_h1 = tf.nn.relu(tf.matmul(self.gnet_z, self.gnet_W1) + self.gnet_b1)
        self.gnet_g = tf.nn.sigmoid(tf.matmul(self.gnet_h1, self.gnet_W2) + self.gnet_b2)

        # Genuine images
        self.x_images = tf.placeholder(dtype=tf.float32, shape = [None, rows, cols, 1])
        self.x_flat = tf.reshape(self.x_images, shape = [-1, rows*cols])
        ...
        self.dnet_W1 = tf.Variable(tf.truncated_normal(shape = [784, 128], stddev = 0.02))
        self.dnet_b1 = tf.Variable(tf.constant(value = 0.0, shape=[128]))
        self.dnet_W2 = tf.Variable(tf.truncated_normal(shape = [128, 1], stddev = 0.02))
        self.dnet_b2 = tf.Variable(tf.constant(value = 0.0, shape=[1]))
        ...
        self.dnet_h1_d = tf.nn.relu(tf.matmul(self.x_flat, self.dnet_W1) + self.dnet_b1)
        self.dnet_y_logit_d = tf.matmul(self.dnet_h1_d, self.dnet_W2) + self.dnet_b2

        self.dnet_h1_g = tf.nn.relu(tf.matmul(self.gnet_g, self.dnet_W1) + self.dnet_b1)
        self.dnet_y_logit_g = tf.matmul(self.dnet_h1_g, self.dnet_W2) + self.dnet_b2

        self.y_dnet = tf.concat([tf.ones_like(self.dnet_y_logit_d), tf.zeros_like(self.dnet_y_logit_g)], axis=0)
        self.y_hat_dnet = tf.concat([self.dnet_y_logit_d, self.dnet_y_logit_g], axis=0)
        self.dnet_cross_entropy = tf.losses.sigmoid_cross_entropy(multi_class_labels = self.y_dnet, \
                                                                  logits = self.y_hat_dnet)

        self.dnet_correct_on_genuine = tf.greater(self.dnet_y_logit_d, 0.5)
        self.dnet_correct_on_fake = tf.less_equal(self.dnet_y_logit_g, 0.5)
        ...
        self.dnet_cross_entropy_g = -1.0*tf.losses.sigmoid_cross_entropy(multi_class_labels = tf.zeros_like(self.dnet_y_logit_g), \
                                                                    logits = self.dnet_y_logit_g)
        ...
        self.d_variables = [self.dnet_W1, self.dnet_b1, self.dnet_W2, self.dnet_b2]
        self.g_variables = [self.gnet_W1, self.gnet_b1, self.gnet_W2, self.gnet_b2]
    ...
    def get_random_noise(self, batch_size):
        return np.random.uniform(-1.0, 1.0, size=[batch_size, 100])

    def train_discriminator(self):
        start_time = time.time()
        training_batch = self.training_data.getNextTrainingBatch(self.batch_size)[0]
        generator_noise = self.get_random_noise(self.batch_size)
    ...
    def generateSingleExample(self):
        generator_noise = self.get_random_noise(1)
        gnet_output = self.gnet_g.eval(feed_dict = {self.gnet_z : generator_noise})
        return np.reshape(gnet_output[0], newshape=[28, 28])
    
    def plot_generated_images(self):
        generator_noise = self.get_random_noise(16)
        g = self.gnet_g.eval(feed_dict = {self.gnet_z : generator_noise})
        f, ax = plt.subplots(4, 4)
        for i in range(4):
            for j in range(4):
                a = ax[i][j]
                a.axis('off')
                im = np.reshape(g[4*i + j], newshape = [28, 28])
                a.imshow(im, interpolation = 'none', cmap = 'gray')
        #plt.show()
        return f
